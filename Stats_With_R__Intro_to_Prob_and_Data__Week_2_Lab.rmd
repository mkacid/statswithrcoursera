...
<li> __No flight is delayed more than 2 hours.__ </li> 
...
ggplot(sfo_feb_flights, aes(x=arr_delay, y=)) +
           geom_histogram(binwidth=15)
```
...
<li> __Delta and United Airlines__ </li>
...
sfo_feb_flights %>%
    group_by(carrier) %>%
    summarize(sfo_feb_median = median(arr_delay), sfo_feb_IQR = IQR(arr_delay)) %>%
    arrange(desc(sfo_feb_IQR))
...
<li> __Median would be more reliable as the distribution of delays is skewed.__ </li> 
...
<li> __There is an overall postive association between distance and average speed.__</li> 
